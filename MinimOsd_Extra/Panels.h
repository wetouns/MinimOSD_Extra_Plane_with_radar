...
byte NOINLINE radar_char() {
  ...
  //    if(is_alt(p)) {
  //        i = normalize_angle(osd_home_direction+180);
  //    } else {
  i = osd_heading;
  //    }
  i = (2 * i + 45) / 90;
...
static void panHorizon(point p) {
  ...
  if (is_alt3(p)) //这里本来参数是5,2，但是这样雷达会跑偏，所以X方向加1，修复位置
    showRADAR(6, 2, is_alt4(p));
...
static void panCOG(point p) {
  ...
  // 这里改成了方位角
  showArrow(grad_to_sect(off_course));
  //wetouns resume
...
static void panFdata(point p) { // итоги полета
  ...
  //这里修复总结界面时间还在跑的问题，新建一个变量，如果变量还没有值，就用飞行时间来赋值，如果已经有了，就不再赋值，保证其有了值之后不再变化
  if (final_flight_time_milis <= 0)
    final_flight_time_milis = total_flight_time_milis;

  printTimeCnv(&final_flight_time_milis, 0);
...
as_char:
...

    {
      ...
      cd = channelDiff(1);
      if ( cd >  150) {
        move_menu(1);    // переходы по строкам по верх-низ
        return;
      }
      if ( cd < -150) {
        move_menu(-1);
      ...
      cd = channelDiff(3);
      if (cd >  150) {
        move_screen(1);   // переходы по экранам - левый дж лево-право
        return;
      }
      if (cd < -150) {
        move_screen(-1);
...
#define RADAR_CHAR 0x1F // code of the radar symbol
  ...
  void writePanels(unsigned long pt) {
    ...
    //DBG_PRINTF("armed=%d", lflags.motor_armed);
    ...
    //if (FLAGS.results_on) {
    // 修复飞行结束后不会显示飞行总结数据的问题,如果已经降落，并且降落时间已经超过3秒
    if (landed > 0  && time_since(&landed) > 3000)
